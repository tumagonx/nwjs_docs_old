...
**1. Create a local symlink to `libudev.so.1`**
...
As you are only modifying local contents of node-webkit directory, this option should not have an impact on the overall stability of your system. However, note that with this method, when the script's process is killed, its nw child process might not die.
